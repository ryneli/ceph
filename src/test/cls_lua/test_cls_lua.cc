...
TEST_F(ClsLua, WriteFull) {
  ...
  bufferlist blout;
  ASSERT_EQ(blin.length(), ioctx.read(oid, blout, 0, 0));
...
TEST_F(ClsLua, GetXattrs) {
  ...
  ASSERT_EQ(0, reply_output.length());
...
TEST_F(ClsLua, VersionSubop) {
  ...
  ASSERT_GT(reply_output.length(), 0);
